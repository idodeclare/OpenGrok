...
/**
 * Represents an abstract base class for Groovy lexers.
 */
abstract class GroovyLexer extends JFlexSymbolMatcher
        implements JFlexJointLexer, Resettable {

    /**
     * Calls {@link #phLOC()} if the yystate is not COMMENT, GROOVYDOC, or
     * SCOMMENT.
     */
    public void chkLOC() {
        if (yystate() != COMMENT() && yystate() != GROOVYDOC() &&
                yystate() != SCOMMENT()) {
            phLOC();
        }
    }
